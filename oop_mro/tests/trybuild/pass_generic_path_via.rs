...
fn main() {
    ...
    let root: Box<dyn AsClass<Root>> =
        Box::new(Nested::<u8>::new()).into_base_via::<(Branch<u8>, Right), dyn AsClass<Root>>();
    assert_eq!(root.as_base::<Root>().label(), "right");

    let root = match root.downcast::<dyn AsClass<Left>>() {
        Ok(_) => panic!("right path should not downcast to left"),
        Err(root) => root,
    };
    let branch = match root.downcast::<dyn AsClass<Branch<u8>>>() {
        Ok(branch) => branch,
        Err(_) => panic!("right path should downcast to branch"),
    };
    let nested = match branch.downcast::<dyn AsClass<Nested<u8>>>() {
        Ok(nested) => nested,
